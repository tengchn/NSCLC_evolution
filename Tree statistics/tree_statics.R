...
files<-dir(path = "cellphy_results", full.names=TRUE) 
cellphy_results<-files[grep("supportTBE",files)]
...
Patients_stage<-read.csv("Patients_Stage.csv", header= T) %>% mutate(across(Patient, ~ as.character(.x)))
...
results <- result %>% mutate_at(vars(-Patient), ~ round(as.numeric(as.character(.x)),5)) %>% 
  ...
  left_join(., select(Patients_stage,Patient,stage,type,status,Filtered_cells)) %>% 
  mutate(cells=paste0(Patient," (",Filtered_cells," cells)")) %>% select(-Filtered_cells)
